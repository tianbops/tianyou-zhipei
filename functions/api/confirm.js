...
export async function onRequest({ request, env }) {
  ...
  try {
    ...
    await redisSet(env, `today_orders:${ROUTE}:${date}`, todayData);
...
async function saveHistory(env, date, today) {
  ...
  const record = { orderBatchId: today.orderBatchId, date, route: ROUTE, vehicle: today.vehicle, count: today.count, weight: today.totalWeight, totalWeight: today.totalWeight, matchedCount: today.matchedCount, newStoreCount: today.newStoreCount, recognizedCount: today.recognizedCount, rawOrderCount: today.rawOrderCount, orders: today.orders, source: today.source, updatedAt: today.updatedAt };
