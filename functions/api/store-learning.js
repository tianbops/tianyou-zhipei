...
export async function onRequest({ request, env }) {
  ...
  try {
    ...
    const rawName = clean(String(body.rawName || ''));
    const baseName = clean(String(body.baseName || ''));
    const baseCode = clean(String(body.baseCode || ''));
    ...
    const target = base.find(item => matchKey(item.name) === matchKey(baseName) || (baseCode && String(item.code || '') === baseCode));
    if (!target) return json({ success: false, error: '确认的基准门店不属于当前线路' }, 400);
    ...
    return json({ success: true, data: { route, rawName, baseName: target.name, learned: true } });
  } catch (error) {
...
function clean(value) {
  return String(value || '').replace(/^[\s\d]+[、.．)）-]+/, '').replace(/\s+/g, ' ').trim();
}
