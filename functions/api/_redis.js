const COOKIE="zp_session";const PREFIX="zp:";
...
export async function command(env,parts){const r=redis(env);if(!r)throw new Error("REDIS_NOT_CONFIGURED");const response=await fetch(r.url,{method:"POST",headers:{Authorization:"Bearer "+r.token,"Content-Type":"application/json"},body:JSON.stringify(parts)});if(!response.ok)throw new Error("REDIS_ERROR");const data=await response.json();return data.result}
