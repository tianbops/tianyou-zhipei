...
请观察整张图片，从顶部一直读取到图片底部。
配送链是最重要区域，必须完整读取到最后一个可见门店，不能提前停止。
对每个门店逐字符检查：中文、英文、数字、Q/JM/A编号、括号和箭头。
遇到小字时仔细检查字符笔画；如果字符确实无法确认，只保留能确认的部分，不要凭常识补字。
...
export async function onRequest({request,env}){
  ...
  try{
    ...
    return json({success:true,data:{route,date:normalizeDate(parsed.date),vehicle:normalizeVehicle(parsed.vehicle),totalWeight:normalizeWeight(parsed.totalWeight),rawOrderCount:0,rawText,mode:'full',message:'图片文字提取完成，请先检查OCR原文。'}});
  }catch(e){
...
async function runVisionOCR(AI,images){
  ...
  try{
    const content=[{type:'text',text:OCR_PROMPT},...dataUrls.map(url=>({type:'image_url',image_url:{url}}))];
    const result=await AI.run(OCR_MODEL,{image:base64,messages:[{role:'user',content}],max_completion_tokens:OCR_MAX_TOKENS,temperature:0,chat_template_kwargs:{enable_thinking:false}});
    ...
    try{
      const result=await AI.run('@cf/llava-hf/llava-1.5-7b-hf',{image:Array.from(payloads[0]),prompt:OCR_PROMPT,max_tokens:OCR_MAX_TOKENS,temperature:0});
      const text=cleanRawText(extractAIText(result));
      if(hasUsableOCR(text))return extractMeta(text);
