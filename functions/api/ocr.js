// 天友智配One - 运单截图 OCR
// 服务器主导：线路从 Session 确定；OCR 只使用当前线路独立基准库。
...
export async function onRequest({request,env}){
  ...
  try{
    ...
    const route=session.role==='admin'?normalizeRoute(requestedRoute):sessionRoute;
...
async function getBaseStores(env,route){
  const key=`route:${normalizeRoute(route)}:base`,url=env.UPSTASH_REDIS_REST_URL,token=env.UPSTASH_REDIS_REST_TOKEN;
  if(!url||!token)throw new Error('服务器基准数据库不可用');
  const r=await fetch(`${url.replace(/\/$/,'')}/get/${encodeURIComponent(key)}`,{headers:{Authorization:`Bearer ${token}`},cache:'no-store'});
  if(!r.ok)throw new Error('线路基准数据库读取失败');
  const d=await r.json();let p=d?.result;try{p=typeof p==='string'?JSON.parse(p):p}catch(_){p=null}
  if(!Array.isArray(p?.stores)||!p.stores.length)throw new Error(`未找到${normalizeRoute(route)}独立基准数据库`);
  return p.stores;
}
