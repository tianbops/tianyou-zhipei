...
export async function onRequest({request,env}) {
  ...
  try{
    ...
    const user={id,name:name||route,route,password:unifiedPassword,role:'driver',createdAt:new Date().toISOString(),sessionVersion:1};
    users.push(user);
...
async function writeJson(env,key,value){
  const r=await fetch(`${env.UPSTASH_REDIS_REST_URL}/set/${encodeURIComponent(key)}`,{method:'POST',headers:{Authorization:`Bearer ${env.UPSTASH_REDIS_REST_TOKEN}`,'Content-Type':'application/json'},body:JSON.stringify(JSON.stringify(value)),cache:'no-store'});
  if(!r.ok)throw Error('save failed');
  const d=await r.json().catch(()=>null);
  if(!d||d.result!=='OK')throw Error('save not confirmed');
}
