// 天友智配One - 服务器会话
// Session 使用 HttpOnly Cookie；线路、用户与车辆信息来自服务器用户资料。
const SESSION_TTL = 8 * 60 * 60;
const SESSION_COOKIE = 'ty_session';

function getSessionSecret(env) { return String(env.SESSION_SECRET || ''); }
function base64url(bytes) { let raw = ''; for (const b of bytes) raw += String.fromCharCode(b); return btoa(raw).replace(/\+/g, '-').replace(/\//g, '_').replace(/=+$/g, ''); }
function decodeBase64url(value) { const pad = value.length % 4 ? '='.repeat(4 - value.length % 4) : ''; const raw = atob(value.replace(/-/g, '+').replace(/_/g, '/') + pad); return Uint8Array.from(raw, c => c.charCodeAt(0)); }
...
function readCookie(request, name) { const header = request.headers.get('Cookie') || ''; for (const part of header.split(';')) { const [key, ...value] = part.trim().split('='); if (key === name) return decodeURIComponent(value.join('=')); } return ''; }
