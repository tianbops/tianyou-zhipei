...
export async function onRequest({request,env}){
 const url=new URL(request.url),route=normalizeRoute(url.searchParams.get('route')),method=request.method;
 ...
 try{
  if(method==='GET'){
   ...
   if(!r.ok)return json({route,stores:[]});const d=await r.json();let stores=[];if(d.result){try{const p=JSON.parse(d.result);stores=Array.isArray(p?.stores)?p.stores:[]}catch{}}
   return json({route,stores});
  }
  if(method==='PUT'){
   const session=await authRequired(request,env,{route});if(!session)return json({error:'未登录或无权修改该线路'},401);
   const body=await request.json();if(!Array.isArray(body.stores))return json({error:'stores 必须是数组'},400);
   ...
   if(!r.ok)return json({error:'Failed to save route'},500);return json({success:true,route,storeCount:body.stores.length});
  }
