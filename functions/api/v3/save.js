...
import { planKey, latestPlanKey, todayWaybillKey, todayCorrectionKey, todayLatestKey, acquireRouteDateLock, releaseRouteDateLock } from './data.js';
export async function savePlan(env,result){
 ...
 try{
  ...
  const todayLatest=todayLatestKey(result.route,result.date);
  ...
  const script="if redis.call('EXISTS',KEYS[1])==1 then return 'EXISTS' end redis.call('SET',KEYS[1],ARGV[1]) redis.call('SET',KEYS[2],ARGV[1]) redis.call('SET',KEYS[3],ARGV[2]) redis.call('SET',KEYS[4],ARGV[1]) redis.call('SET',KEYS[5],ARGV[2]) return 'OK'";
  const outcome=await evalRedis(env,script,[key,latestKey,waybillKey,correctionKey,todayLatest],[JSON.stringify(result),JSON.stringify(correctionData)]);
