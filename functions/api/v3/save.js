...
import { planKey, latestPlanKey, acquireRouteDateLock, releaseRouteDateLock } from './data.js';
export async function savePlan(env,result){
 ...
 try{
  ...
  const script="if redis.call('EXISTS',KEYS[1])==1 then return 'EXISTS' end redis.call('SET',KEYS[1],ARGV[1]) redis.call('SET',KEYS[2],ARGV[1]) return 'OK'";
  const outcome=await evalRedis(env,script,[key,latestPlanKey(result.route)],[JSON.stringify(result)]);
