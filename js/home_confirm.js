  ...
  async function confirm(){
    ...
    try{
      if(!parsedState.length){
        if(!String(input?.value||'').trim())throw Error('请先点击“规划”');
      ...
      if(route!==String(metaState.route||'').trim())throw Error(`调度线路已从 ${metaState.route||'未选择'} 切换为 ${route||'未选择'}，当前结果已失效，请重新规划`);
      if(!route){
