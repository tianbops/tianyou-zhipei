...
window.clearManualInput=()=>{if(typeof window.clearUploadedImage==='function')window.clearUploadedImage();const input=$('manualOrderInput');if(input){input.value='';input.setAttribute('placeholder','上传图片后，这里显示识别文字。请核对后再开始解析，也可直接粘贴运单文字。')}if($('charCount'))$('charCount').textContent='0';parsedOrders=[];pendingMeta={};reviewMode=false;const status=$('parseStatus');if(status){status.classList.remove('active');if($('statusIcon'))$('statusIcon').textContent='⏳';if($('statusText'))$('statusText').textContent='等待处理...';if($('progressBar'))$('progressBar').style.width='0%';if($('statusCount'))$('statusCount').textContent=''}window.renderReviewStores?.([])};
