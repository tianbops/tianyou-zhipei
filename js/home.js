...
(function () {
  ...
  async function loadBaseStores() {
    ...
    if (!Array.isArray(stores) || !stores.length) throw new Error('当前线路暂无基准门店数据');
    baseStores = stores;
    return baseStores;
  }
  ...
  function rankOrders(list) {
    const rank = new Map();
    baseStores.forEach((s,i)=>{ const name=storeName(s); if(name) rank.set(normalizeName(name),i); if(s?.code) rank.set(String(s.code),i); });
    return (Array.isArray(list)?list:[]).map((item,i)=>({...item,_i:i})).sort((a,b)=>{
      const an=normalizeName(a?.name||a?.storeName||a?.shopName||a?.门店名称), bn=normalizeName(b?.name||b?.storeName||b?.shopName||b?.门店名称);
  ...
  function updateSummary() {
    ...
    if ($('storeCount')) $('storeCount').textContent = count ? `${count}家门店` : '暂无今日订单';
    if ($('totalWeight')) $('totalWeight').textContent = weight ? `${Number(weight.toFixed(2))} kg` : '暂无数据';
  ...
  window.loadExampleData=function(){const names=baseStores.slice(0,3).map(storeName).filter(Boolean);$('manualOrderInput').value=names.length?names.join('\n'):'江北胡汪洋经销商\n中景隆贸易\n江北重庆兴农';$('manualOrderInput').dispatchEvent(new Event('input'));};
  window.pasteFromClipboard=async function(){try{$('manualOrderInput').value=await navigator.clipboard.readText();$('manualOrderInput').dispatchEvent(new Event('input'));}catch(_){toast('无法读取剪贴板，请手动粘贴','warning');}};
