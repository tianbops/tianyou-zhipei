...
window.clearManualInput=()=>{if(parseAbortController){parseCancelled=true;parseAbortController.abort();}if(typeof window.cancelOCR==='function')window.cancelOCR().catch(()=>{});const input=$('manualOrderInput');if(input){input.value='';input.setAttribute('placeholder','上传图片后，这里显示识别文字。请核对后再开始解析，也可直接粘贴运单文字。')}parsedOrders=[];pendingMeta={};reviewMode=false;const status=$('parseStatus');if(status){status.classList.remove('active','loading','success','error','cancelled');if($('statusIcon'))$('statusIcon').className='status-icon';if($('statusText'))$('statusText').textContent='等待处理...';if($('progressBar'))$('progressBar').style.width='0%'}window.renderReviewStores?.([])};
window.pasteFromClipboard=async()=>{try{if(parseAbortController){parseCancelled=true;parseAbortController.abort();}if(typeof window.cancelOCR==='function')window.cancelOCR().catch(()=>{});const text=await navigator.clipboard.readText();setOCRText(text);pendingMeta={};reviewMode=false}catch(_){toast('无法读取剪贴板，请手动粘贴','warning')}};
window.parseManualInput=async()=>{if(parseInFlight)return toast('解析正在进行，请勿重复点击','warning');try{const text=$('manualOrderInput')?.value||'';if(!text.trim())return toast('请先输入或识别运单文字','warning');window.renderUnifiedStatus('loading',8,'正在整理运单数据...');await new Promise(resolve=>setTimeout(resolve,0));window.renderUnifiedStatus('loading',32,'正在匹配配送门店...');const data=await parseOrderText(text);window.renderUnifiedStatus('loading',78,'正在生成今日路线...');parsedOrders=Array.isArray(data.stores)?data.stores:[];pendingMeta={date:data.date||pendingMeta.date||'',vehicle:data.vehicle||pendingMeta.vehicle||'',totalWeight:data.totalWeight||pendingMeta.totalWeight||'',rawOrderCount:Number(data.rawOrderCount)||0,matchedCount:Number(data.matchedCount)||0,newStoreCount:Number(data.newStoreCount)||0,reviewCount:Number(data.reviewCount)||0,duplicateCount:Number(data.duplicateCount)||0,recognizedCount:Number(data.recognizedCount)||0,uniqueStoreCount:Number(data.uniqueStoreCount)||parsedOrders.length,baseDatabaseAvailable:data.baseDatabaseAvailable!==false,source:'web-confirm'};const uniqueCount=Number(data.uniqueStoreCount)||parsedOrders.length;const rawCount=Number(data.rawOrderCount)||Number(data.recognizedCount)||parsedOrders.length;const message=data.baseDatabaseAvailable===false?`解析完成：未建立${currentRoute()}基准库，按运单识别顺序排列 ${uniqueCount} 家门店，总重量 ${data.totalWeight||'未识别'}`:(parsedOrders.length?`解析完成：${uniqueCount} 家唯一门店，原始记录 ${rawCount} 条${data.newStoreCount?`，新增 ${data.newStoreCount} 家`:''}${data.duplicateCount?`，合并重复 ${data.duplicateCount} 条`:''}`:'没有识别到有效门店');window.onOrderParsed?.(data);setReviewText(data);window.renderUnifiedStatus(parsedOrders.length?'success':'error',100,parsedOrders.length?message:'没有识别到有效门店');
const detailLines=[];
if(parsedOrders.length){
  ...
  detailLines.push(`规划完成：现${uniqueCount}家，原${rawCount}家，合并${mergeCount}家，更正${correctionCount}家`);
