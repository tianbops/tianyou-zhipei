...
window.toggleOCRText=()=>{const sheet=document.querySelector('.upload-sheet');const toggle=$('ocrTextToggle');if(!sheet||!toggle)return;const open=sheet.classList.toggle('ocr-text-open');toggle.setAttribute('aria-expanded',open?'true':'false');toggle.querySelector('span').textContent=open?'⌄':'›';const status=$('parseStatus');if(status)status.classList.toggle('detail-hidden',open);};
...
function openCorrectionDetails(){
  ...
  list.textContent='';
  correctionDetails.forEach(item=>{const row=document.createElement('div');row.className='correction-item';row.textContent=String(item);list.appendChild(row);});
...
const correctionClose=$('correctionClose');if(correctionClose)correctionClose.addEventListener('click',()=>{$('correctionModal')?.classList.remove('active');$('correctionModal')?.setAttribute('aria-hidden','true')});const correctionModal=$('correctionModal');if(correctionModal)correctionModal.addEventListener('click',event=>{if(event.target===correctionModal){correctionModal.classList.remove('active');correctionModal.setAttribute('aria-hidden','true')}});const cached=readCachedHomeOrder();if(cached){serverOrder=cached;updateSummary()}if(!(await Auth.checkAuth()))return;await loadServerOrder(currentDate());writeCachedHomeOrder(serverOrder);updateSummary();$('manualOrderInput')?.addEventListener('input',function(){if(reviewMode){reviewMode=false;parsedOrders=[];setPrimaryActionMode('plan');window.onOrderParsed?.({stores:[]});window.renderUnifiedStatus('idle',0,'订单信息已修改，请重新规划')}});document.addEventListener('click',event=>{const menu=$('homeMenu'),button=document.querySelector('.menu-btn');if(menu&&menu.style.display==='block'&&!menu.contains(event.target)&&!button?.contains(event.target))menu.style.display='none'})}catch(e){console.error('首页初始化失败',e);error(e.message||'首页初始化失败')}});
window.addEventListener('pageshow',event=>{document.body.classList.remove('is-leaving');const menu=$('homeMenu');if(menu)menu.style.display='none';const overlay=$('uploadOverlay');if(overlay)overlay.classList.remove('active');closeUploadSource();if(event.persisted&&typeof Auth!=='undefined')refreshHomeOrder()});
})();
