...
async function loadServerOrder(date=''){const route=currentRoute();if(!route)throw Error('未指定配送线路');const params=new URLSearchParams();params.set('route',route);if(date)params.set('date',date);const query=params.toString();const controller=new AbortController();const timer=setTimeout(()=>controller.abort(),10000);try{const response=await fetch(`/api/orders${query?`?${query}`:''}`,{cache:'no-store',credentials:'same-origin',signal:controller.signal});if(!response.ok)throw Error(response.status===401?'登录已失效，请重新登录':`当日订单读取失败（${response.status}）`);const data=await response.json();const incoming=data?.today;const incomingValid=Boolean(incoming&&Array.isArray(incoming.orders)&&incoming.orders.length);if(incomingValid){serverOrder=incoming;serverOrder._todayWaybillCount=Math.max(0,Number(data?.todayWaybillCount)||0);serverOrder._todaySummary=data?.todaySummary||null;}else if(!serverOrder||!Array.isArray(serverOrder.orders)||!serverOrder.orders.length){serverOrder=null;}else{serverOrder._todayWaybillCount=Math.max(0,Number(data?.todayWaybillCount)||serverOrder._todayWaybillCount||0);serverOrder._todaySummary=data?.todaySummary||serverOrder._todaySummary||null;}return serverOrder}catch(error){if(error?.name==='AbortError')throw Error('当日任务读取超时，请稍后重试');throw error}finally{clearTimeout(timer)}}
