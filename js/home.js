...
async function loadDispatchRoutes(){const select=$('dispatchRouteSelect');if(!select)return;const response=await fetch('/api/routes',{cache:'no-store',credentials:'same-origin'});const data=await response.json().catch(()=>({}));if(!response.ok||!data.success)throw Error(data.error||'路线列表读取失败');const available=(Array.isArray(data.routes)?data.routes:[]).map(x=>String(x.id||x.name||'').trim()).filter(Boolean);const bound=Auth.getBoundRoute?Auth.getBoundRoute():Auth.getCurrentRoute();if(bound&&!available.includes(bound))available.unshift(bound);const selected=currentRoute();select.innerHTML=available.map(route=>`<option value="${route}">${route}</option>`).join('');if(selected&&available.includes(selected))select.value=selected;else if(available[0]){Auth.setDispatchRoute?.(available[0]);select.value=available[0];}select.onchange=()=>{Auth.setDispatchRoute?.(select.value);location.reload()};if($('menuRoute'))$('menuRoute').textContent=select.value||'未选择线路'}
...
function homeCacheScope(){const user=typeof Auth!=='undefined'?(Auth.serverUser||{}):{};const identity=String(user.id||user.username||user.account||user.route||'').trim();const route=String(currentRoute()||'').trim();return identity&&route?HOME_ORDER_CACHE_PREFIX+':'+encodeURIComponent(identity)+':'+encodeURIComponent(route):''}
