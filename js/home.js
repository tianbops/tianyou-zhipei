...
function setPrimaryActionMode(mode){primaryActionMode=['confirm','retry'].includes(mode)?mode:'plan';const button=$('primaryActionBtn');if(!button)return;button.textContent=primaryActionMode==='confirm'?'确认录入':primaryActionMode==='retry'?'重新上传':'规划路线';button.classList.toggle('ready',primaryActionMode==='confirm');button.disabled=false}
